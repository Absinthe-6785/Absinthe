...
| frontend/src/lib/noteIndexedDb.ts: NOTES_IDB_NAME and legacy mutations | Current production Notes path is absinthe-notes-v1. | It is not a K-334 authority store and cannot natively transact with a separate database. | Current production legacy path. |
| frontend/src/lib/notePersistence.ts: legacy replacement guards | Coordinates legacy IndexedDB/localStorage persistence and recovery checks. | Existing fallback/localStorage operations are not atomic durable-authority commits. | Current production legacy path. |
...
| Candidate operation | Required atomic participants / boundary | Retry outcome |
|---|---|---|
| Accept root/successor | Subject, predecessor/head comparison, next sequence, immutable event, optional projection/quarantine. | One native transaction; exact replay idempotent. |
| Preserve proposal | Proposal, predecessor relation, conflict group/projection. | No head transition without owner resolution. |
| Detect accepted fork | Event, predecessor relation, successor lookup/projection, quarantine. | Reopen cannot select a transient winner. |
| Record grants/termination | Evidence and exact scope/lifecycle indexes. | Never create usable projection without evidence. |
| Resolve conflict | Resolution evidence and exact conflict relation. | Preserve losers; later acceptance is separately authorized. |
| Record compatibility/mapping | Immutable entry plus lookup indexes. | No partial effective result. |
| Initial source operation authority commit | Future K-332 mutation/revision/admission/outbox/receipt/stage-A evidence. | Needs a later selected same-DB boundary; none claimed today. |
| Terminal reconciliation | Terminal evidence bound to exact initial digest. | Separate only with durable intent and independent revalidation. |
| Rebuild projection | Validated immutable evidence only. | Cannot create authority or resolve missing lineage. |
| Import/restore/replay | Imported evidence, provenance, dependency state, receipt. | Cannot auto-accept/map/authorize/activate. |

Native atomicity is limited to one selected IndexedDB transaction. Legacy IndexedDB, localStorage, K-328, attachments, and remote state are not one transaction.

## 16. Failure-Atomicity Matrix

| Failure/observation | Preserve / accepted state | Blocked action and retry safety | Open implementation question |
|---|---|---|---|
| Crash before transaction | No new acceptance. | Retry reads durable state. | Intent timing. |
| Crash during transaction | Abort leaves no partial acceptance/projection. | Strict reread/retry safe. | Transaction API/error mapping. |
| Crash after commit before acknowledgement | Exact immutable evidence remains. | Exact retry idempotent. | Receipt form/retention. |
| Duplicate/concurrent retry | One valid line or preserved conflict. | Loser rereads/fails or preserves deterministically. | CAS/index strategy. |
| Stale/missing predecessor | No acceptance/head. | Retry only after lineage exists. | Candidate retention boundary. |
| Missing authority | No authority-dependent acceptance. | Preserve safe candidate; fail closed. | Grant lifecycle evaluator. |
| Incompatible tuple | No authority-dependent use. | Retain evidence, bounded result. | Tuple/range semantics. |
| Missing mapping | No mapping-dependent issuance. | Explicit mapping required. | Mapping lifecycle. |
| Competing proposal | No head advance. | Owner resolution required. | Resolution placement. |
| Late accepted fork | All branches retained/quarantined. | Subject blocked; no auto-repair. | Marker choice. |
| Malformed retainable evidence | Safe bounded provenance/reference only. | Strict reads fail closed. | Privacy/retention policy. |
| Quota/index failure | No partial transition. | Preserve old history; bounded failure/retry. | Capacity preflight. |
| Upgrade interruption | No migration-derived authority. | Reopen/retry versioned upgrade. | Upgrade rollback. |
| Recovery replay/import | Revalidate evidence/dependencies. | No authority manufacture. | Trust/provenance. |
| Restore over newer evidence | Preserve all provenance/evidence. | No timestamp overwrite/branch selection. | Reconciliation authority. |
