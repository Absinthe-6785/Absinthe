...
| `NO_EFFECTIVE_SELECTION` | `ROLLBACK_DECISION` | exact current unique `REVOKE` head | only with future rollback authorization | `ACTIVE_SELECTION` using a historically known digest | next | same subject; no later successor; historical target; exact canonical bytes; digest/source/version/compatibility revalidation | does not erase the revocation interval, restart history, or retroactively make that interval active | `ROLLBACK_NOT_AUTHORIZED`, `ROLLBACK_TARGET_NOT_HISTORICAL`, `PREDECESSOR_NOT_CURRENT_HEAD`, `SEQUENCE_NOT_NEXT`, or selected-byte/compatibility failure |
...
After `NO_EFFECTIVE_SELECTION`, `SUPERSEDE` remains the ordinary successor for a newly approved selection, while `ROLLBACK_DECISION` is the separately authorized successor for intentionally reactivating a historically used digest. The latter extends the exact revocation head, preserves the entire revoked interval, and never mutates/deletes revocation, rewinds a pointer, restores a database snapshot as authority, creates a new genesis `SELECT`, or bypasses rollback authorization. `SELECT` remains genesis-only after revocation.
...
| `ISSUER_NOT_AUTHORIZED`, `ROLLBACK_NOT_AUTHORIZED` | denied evidence; issuance authorization | Proposed issuer does not satisfy the future owner-approved policy; issue no lifecycle event. | Retain bounded attempted-evidence diagnostics where permitted; no authority is created. | Selection/History |
...
Conceptual future validation/detection order is: exact subject; action transition; issuer authorization; predecessor existence/current-head status; exact next sequence; target digest/history conditions; authoritative issuance; concurrent conflict; persistent history resolution; fork detection; derived current selection; duplicate-effective-selection detection. One event may produce several diagnostics; enforcement uses the most safety-relevant blocking state. This ordering grants no authority and implements no validator, allocator, transaction, lock, CAS, or consensus.
...
| Rollback mechanics | none | authorized successor to historically known digest from active or revocation head; revoked interval remains historical | DEFINED_FOR_FUTURE_CONTRACT | K-333 Manifest Selection and History Authority | same subject | append-only | full graph/bytes | history/source/compatibility/current head | deny if invalid | boundary input later | no database rollback or retroactive revival | none | Rollback Contract |
