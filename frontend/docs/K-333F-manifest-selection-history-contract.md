...
Conceptual future validation/detection order is: exact subject; action transition; predecessor existence/current-head status; exact next sequence; complete accepted-authoritative-lineage membership and action classification; target bytes and compatibility; rollback authorization when classification requires it; authoritative issuance; concurrent conflict; persistent history resolution; fork detection; derived current selection; duplicate-effective-selection detection. One event may produce several diagnostics; enforcement uses the most safety-relevant blocking state. Membership/classification precedes selected bytes and rollback authorization; unavailable membership is never treated as absent. This ordering grants no authority and implements no validator, allocator, transaction, lock, CAS, consensus, lineage index, or database query.
...
| relabel bypass, false rollback, incomplete-history downgrade, or cross-subject replay | exact-subject complete-lineage predicate; mutually exclusive action classification; unknown fails closed | reject or unavailable; no state-changing event | future Selection/History and persistence authority remain required |
