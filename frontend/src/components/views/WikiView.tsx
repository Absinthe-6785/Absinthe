...
function parseMarkdown(md: string, allNotes: Note[]): string {
  ...
  html = html.replace(/%%MATH(\d+)%%/g, (_, idx: string) => {
...
const CSS = `
  ...
  .w-sidebar{width:190px;min-width:190px;background:#141416;border-right:1px solid #222;display:flex;flex-direction:column}
  .fi{display:flex;align-items:center;gap:7px;padding:6px 11px;cursor:pointer;transition:background .15s;font-size:12px}
  .fi:hover{background:#1C1C1E}.fi.active{background:#1C1C1E;border-right:2px solid #FACC15}.fi.active .fil{color:#FACC15}
  .ni{padding:8px 10px;cursor:pointer;border-bottom:1px solid #181818;transition:background .15s}
  .ni:hover{background:#181818}.ni.active{background:#1C1C1E;border-left:2px solid #FACC15}
  ...
  .md-p{margin:4px 0}.md-bold{color:#FACC15;font-weight:700}.md-italic{color:#D4C99A;font-style:italic}
  ...
  .math-inline{display:inline}.math-err{color:#F87171;font-family:'JetBrains Mono',monospace;font-size:12px}
  ...
  .wbg{background:#FACC15;color:#0F0F11;border:none;border-radius:7px;padding:6px 12px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit}
  .wbg:hover{background:#FDE047}
  .wsi{background:#181818;border:1px solid #1E1E20;border-radius:16px;padding:6px 10px 6px 28px;font-size:12px;color:#E8E6E0;outline:none;width:100%;font-family:inherit}
  .wsi:focus{border-color:#FACC1550}
  .wbadge{background:#FACC1520;color:#FACC15;border-radius:999px;font-size:10px;font-weight:700;padding:1px 6px}
  .wbadger{background:#EF444420;color:#F87171;border-radius:999px;font-size:10px;font-weight:700;padding:1px 6px}
  .sec-lbl{padding:8px 11px 3px;font-size:10px;color:#3A3A3A;font-weight:700;letter-spacing:1px;text-transform:uppercase}
  ...
  .tag-pill:hover{border-color:#8B5CF660}.tag-pill.active{background:#8B5CF640;border-color:#8B5CF6}
  .bl-item{padding:6px 10px;font-size:12px;color:#60A5FA;cursor:pointer;border-radius:5px;transition:background .15s}
  .bl-item:hover{background:#1C1C1E}
  .save-ind{font-size:9px;color:#4ADE8080;font-family:monospace;display:flex;align-items:center;gap:3px}
`;
...
export const WikiView = () => {
  ...
  const [folders, setFolders] = useState<Folder[]>(() => loadLS<Folder[]>(LS_FOLDERS, INIT_FOLDERS));
  const [notes,   setNotes]   = useState<Note[]>(  () => loadLS<Note[]>(  LS_NOTES,   INIT_NOTES));
  const [activeFolderId, setActiveFolderId] = useState<string | null>(null);
  const [activeNoteId,   setActiveNoteId]   = useState<string | null>(
    () => loadLS<Note[]>(LS_NOTES, INIT_NOTES)[0]?.id ?? null
  );
  ...
  const [rightPanel,     setRightPanel]     = useState<'toc' | 'backlinks' | 'tags'>('toc');
  ...
  const autoSave = useCallback((nextNotes: Note[] | null, nextFolders: Folder[] | null) => {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => {
      if (nextNotes)   saveLS(LS_NOTES,   nextNotes);
      if (nextFolders) saveLS(LS_FOLDERS, nextFolders);
      setSavedAt(new Date());
    }, 500);
  }, []);
  ...
  const createNote = useCallback(() => {
    ...
    const note: Note = { id, folderId: fid, title: '새 노트', body: '# 새 노트\n\n#태그\n\n내용을 입력하세요.', updatedAt: Date.now(), starred: false, deletedAt: null };
    const next = [note, ...notes];
    setNotes(next); setActiveNoteId(id); autoSave(next, null);
  }, [activeFolderId, notes, autoSave]);

  const updateNote = useCallback((id: string, patch: Partial<Note>) => {
    const next = notes.map(n => n.id === id ? { ...n, ...patch, updatedAt: Date.now() } : n);
    setNotes(next); autoSave(next, null);
  }, [notes, autoSave]);

  const trashNote = useCallback((id: string) => {
    const next = notes.map(n => n.id === id ? { ...n, deletedAt: Date.now() } : n);
    setNotes(next); if (activeNoteId === id) setActiveNoteId(null); autoSave(next, null);
  }, [notes, activeNoteId, autoSave]);

  const restore = useCallback((id: string) => {
    const next = notes.map(n => n.id === id ? { ...n, deletedAt: null } : n);
    setNotes(next); autoSave(next, null);
  }, [notes, autoSave]);
  ...
  const addFolder = () => {
    ...
    const next: Folder[] = [
      ...folders,
      { id, name: newFolderName.trim(), emoji: newFolderEmoji, color: FOLDER_COLORS[folders.length % FOLDER_COLORS.length] },
    ];
    setFolders(next); setNewFolderName(''); setNewFolderEmoji('📄');
    setShowFolderForm(false); setActiveFolderId(id);
    autoSave(null, next);
  };
  ...
  const insert = (before: string, after = '') => {
    ...
    updateNote(activeNote.id, {
      body: activeNote.body.substring(0, s) + before + sel + after + activeNote.body.substring(e),
    });
  ...
  const handleImageInsert = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = ev => {
      ...
      insert(`![${file.name.replace(/\.[^.]+$/, '')}](${dataUrl})`);
    };
  ...
  const handlePreviewClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const wl = target.closest('.wiki-link') as HTMLElement | null;
    if (wl?.dataset.id) { setActiveNoteId(wl.dataset.id); return; }
    const wt = target.closest('.wiki-tag') as HTMLElement | null;
  ...
  const VIEW_MODES: { key: 'edit' | 'split' | 'preview'; icon: ReactNode }[] = [
    { key: 'edit',    icon: <Edit3 size={11}/> },
    { key: 'split',   icon: <AlignLeft size={11}/> },
    { key: 'preview', icon: <Eye size={11}/> },
  ];

  const RIGHT_PANELS: { key: 'toc' | 'backlinks' | 'tags'; label: string; icon: ReactNode }[] = [
    { key: 'toc',       label: '목차', icon: <AlignLeft size={11}/> },
    { key: 'backlinks', label: '링크', icon: <Link size={11}/> },
    { key: 'tags',      label: '태그', icon: <Tag size={11}/> },
  ];
    ...
    <div className="wiki-wrap">
      ...
      <div className="w-sidebar">
        <div style={{ padding: '13px 11px 9px', borderBottom: '1px solid #1C1C1E', display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <span style={{ marginLeft: 'auto', fontSize: 9, color: '#333', fontFamily: 'monospace' }}>β</span>
        </div>
        <div style={{ padding: '7px 9px', borderBottom: '1px solid #1C1C1E', position: 'relative' }}>
          ...
          <input className="wsi" placeholder="검색..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}/>
        </div>

        <div style={{ flex: 1, overflowY: 'auto' }}>
          <div className={`fi ${activeFolderId === null && !activeTag ? 'active' : ''}`}
            ...
            <span className="fil" style={{ flex: 1 }}>전체 노트</span>
            <span className="wbadge">{notes.filter(n => !n.deletedAt).length}</span>
          </div>

          {starredCount > 0 && (
            <div className={`fi ${activeFolderId === 'starred' ? 'active' : ''}`} onClick={() => setActiveFolderId('starred')}>
              <Star size={12} color={activeFolderId === 'starred' ? '#FACC15' : '#555'} fill={activeFolderId === 'starred' ? '#FACC15' : 'none'}/>
              <span className="fil" style={{ flex: 1 }}>즐겨찾기</span>
              <span className="wbadge">{starredCount}</span>
            </div>
          )}

          <div className="sec-lbl">과목</div>
          {folders.map(f => (
            <div key={f.id} className={`fi ${activeFolderId === f.id ? 'active' : ''}`}
              onClick={() => { setActiveFolderId(f.id); setActiveTag(null); }}>
              <span style={{ fontSize: 13 }}>{f.emoji}</span>
              <span className="fil" style={{ flex: 1 }}>{f.name}</span>
              <span className="wbadge" style={{ background: f.color + '25', color: f.color }}>
          ...
          {showFolderForm ? (
            <div style={{ padding: '5px 9px', display: 'flex', flexDirection: 'column', gap: 5 }}>
              <div style={{ display: 'flex', gap: 4 }}>
                <input className="wi" style={{ width: 30, padding: '4px 5px', textAlign: 'center' }}
                  value={newFolderEmoji} onChange={e => setNewFolderEmoji(e.target.value)} maxLength={2}/>
                <input className="wi" style={{ flex: 1 }} placeholder="과목명"
                  value={newFolderName} onChange={e => setNewFolderName(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') addFolder(); if (e.key === 'Escape') setShowFolderForm(false); }}
                  autoFocus/>
              </div>
              <div style={{ display: 'flex', gap: 4 }}>
                <button className="wbg" style={{ flex: 1, padding: '5px' }} onClick={addFolder}>추가</button>
                <button onClick={() => setShowFolderForm(false)}
                  style={{ flex: 1, background: '#1C1C1E', border: 'none', borderRadius: 6, color: '#666', fontSize: 11, cursor: 'pointer' }}>취소</button>
              </div>
            </div>
          ) : (
            <div className="fi" onClick={() => setShowFolderForm(true)} style={{ color: '#3A3A3A', fontSize: 11 }}>
          ...
          {allTags.length > 0 && (
            <>
              <div className="sec-lbl" style={{ marginTop: 4 }}>태그</div>
          ...
          <div style={{ borderTop: '1px solid #1C1C1E', marginTop: 6 }}>
            <div className={`fi ${isTrash ? 'active' : ''}`} onClick={() => setActiveFolderId('trash')}>
              <Trash2 size={11} color={isTrash ? '#F87171' : '#444'}/>
              <span className="fil" style={{ flex: 1, color: isTrash ? '#F87171' : undefined }}>휴지통</span>
              {trashCount > 0 && <span className="wbadger">{trashCount}</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ width: 190, minWidth: 190, background: '#0D0D0F', borderRight: '1px solid #1C1C1E', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '9px 10px 7px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #1C1C1E' }}>
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            ...
            {!isTrash && (
              <button onClick={createNote} style={{ background: '#FACC15', border: 'none', borderRadius: 5, padding: '2px 7px', cursor: 'pointer', color: '#0F0F11', fontWeight: 700, fontSize: 12, display: 'flex', alignItems: 'center' }}>
                <Plus size={11}/>
              </button>
            )}
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {visibleNotes.length === 0 ? (
            ...
            return (
              <div key={n.id} className={`ni ${n.id === activeNoteId ? 'active' : ''}`} onClick={() => setActiveNoteId(n.id)}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {activeNote ? (
          <>
            <div style={{ padding: '7px 13px', borderBottom: '1px solid #1C1C1E', display: 'flex', alignItems: 'center', gap: 6, background: '#0F0F11', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', background: '#161616', borderRadius: 7, padding: 2, gap: 1 }}>
                {VIEW_MODES.map(({ key, icon }) => (
                  <button key={key} onClick={() => setViewMode(key)} className="tbtn"
                    style={{ padding: '3px 7px', borderRadius: 5, background: viewMode === key ? '#2A2A2A' : 'none', color: viewMode === key ? '#FACC15' : '#444' }}>
                    {icon}
                  </button>
                ))}
              </div>
            ...
            {!isTrash && (viewMode === 'edit' || viewMode === 'split') && (
              <div style={{ padding: '2px 10px', borderBottom: '1px solid #161616', display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0, background: '#0F0F11', flexWrap: 'wrap' }}>
                ...
                {savedAt && (
                  <span className="save-ind" style={{ marginLeft: 'auto' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, color: '#2A2A2A' }}>
            ...
            <button className="wbg" onClick={createNote}>+ 새 노트</button>
          </div>
        )}
      </div>
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid #1C1C1E', flexShrink: 0 }}>
            {RIGHT_PANELS.map(({ key, label, icon }) => (
              <button key={key} onClick={() => setRightPanel(key)}
                style={{ flex: 1, background: 'none', border: 'none', borderBottom: rightPanel === key ? '2px solid #FACC15' : '2px solid transparent', padding: '8px 4px', cursor: 'pointer', color: rightPanel === key ? '#FACC15' : '#444', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, transition: 'all .15s' }}>
                {icon}{label}
              </button>
            ))}
          </div>
          ...
          {rightPanel === 'backlinks' && (
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
              ...
              {backlinks.length === 0
                ...
                : backlinks.map(n => (
                  <div key={n.id} className="bl-item" onClick={() => setActiveNoteId(n.id)}>↗ {n.title}</div>
                ))
              }
              {(() => {
                const outLinks = [...activeNote.body.matchAll(/\[\[(.+?)\]\]/g)].map(m => m[1]);
                const found = outLinks
                  .map(t => notes.find(n => n.title === t && !n.deletedAt))
                  .filter((n): n is Note => n !== undefined);
                return found.length > 0 ? (
                  <>
                    <div style={{ padding: '8px 10px 4px', fontSize: 10, color: '#444', borderTop: '1px solid #1C1C1E', marginTop: 4 }}>나가는 링크</div>
                    {found.map(n => (
                      <div key={n.id} className="bl-item" style={{ color: '#86EFAC' }} onClick={() => setActiveNoteId(n.id)}>→ {n.title}</div>
                    ))}
                  </>
                ) : null;
              })()}
