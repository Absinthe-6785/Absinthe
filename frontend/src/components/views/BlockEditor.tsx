...
const SingleBlock = React.memo(function SingleBlock({
  ...
  const contentBody = needsShell ? body : (
    <div className="be-content" onMouseDown={handleContentMouseDown}>
      {body}
    </div>
  );
...
function BlockEditorInner({ blocks, onChange, colors: c, readOnly, searchQuery, depth,
  ...
  const gutterDragRef = useRef<GutterSelectionState | null>(null);
  ...
  const handleGutterPointerDown = useCallback((blockId: string, e: React.PointerEvent<HTMLDivElement>) => {
    ...
    gutterDragRef.current = beginGutterSelection(blockId, e.pointerId);
    setIsGutterDragging(true);

    const selected = updateGutterSelection(getRootBlocks(), blockId, blockId);
    setSelectedBlockIds(selected);
  ...
  useEffect(() => {
    if (!isGutterDragging || depth !== 0) return;

    const onPointerMove = (e: PointerEvent) => {
      const drag = gutterDragRef.current;
      if (!drag || e.pointerId !== drag.pointerId) return;
      const hoverId = hitTestBlockIdFromPoint(e.clientX, e.clientY, editorRootRef.current);
      if (!hoverId) return;
      const selected = updateGutterSelection(getRootBlocks(), drag.anchorId, hoverId);
      setSelectedBlockIds(selected);
    ...
    const endDrag = (e: PointerEvent) => {
      const drag = gutterDragRef.current;
      if (!drag || e.pointerId !== drag.pointerId) return;
      editorRootRef.current?.releasePointerCapture(e.pointerId);
  ...
  }, [isGutterDragging, depth, getRootBlocks, handleActiveBlockChange]);
