...
import { BookOpen, Briefcase, Dumbbell, Activity, Clock, Calendar, CalendarDays, CheckCircle, Moon, Users, User, ChevronDown } from 'lucide-react';
...
import { AnalyticsProps, Schedule, Routine } from '../../types';
...
const CATEGORY_META: Record<string, { icon: ReactNode; color: string; tw: string }> = {
  ...
  Personal: { icon: <User size={16}/>,     color: 'bg-pink-500',    tw: 'text-pink-500'    },
  Sleep:    { icon: <Moon size={16}/>,      color: 'bg-gray-500',    tw: 'text-gray-400'    },
...
export function LegacyAnalyticsView({
  now, showToast, schedules,
  appSettings, theme, routines, formatDate,
}: AnalyticsProps) {
  const { t } = useTranslation();
  const { confirm, clearConfirm, handleConfirm } = useConfirm();
  ...
  const weeklyReview = useMemo(() => {
    ...
    const today = now.toJSDate();
    const todayStrLocal = formatDate(today);
    const dow = (today.getDay() + 6) % 7;
    const weekStart = new Date(today);
    weekStart.setDate(today.getDate() - dow);
    const weekStartStr = formatDate(weekStart);
    const thisWeek = heatmapData.filter(d => d.date >= weekStartStr && d.date <= todayStrLocal);
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-5 overflow-y-auto lg:overflow-hidden pr-2 pb-20 lg:pb-2">
        <div className="w-full lg:flex-[3.5] flex flex-col gap-5 shrink-0">
          ...
          {heatmapData.length > 0 && (() => {
            const todayStrHeat = formatDate(now.toJSDate());
                      ...
                      {weeks.map((week, wi) => (
                        <div key={wi} className="flex flex-col gap-0.5 flex-1">
                          {week.map((dateStr, di) => {
                            ...
                            const isFuture = dateStr > todayStrHeat;
          ...
          <div className={`rounded-[24px] lg:rounded-[32px] shadow-sm p-5 lg:p-6 flex flex-col relative transition-colors ${theme.card}`}>
            ...
            <div className="flex items-center justify-between gap-1">
              {['Mo','Tu','We','Th','Fr','Sa','Su'].map((day, idx) => {
                ...
                const isTodayCell = dateStr === formatDate(now.toJSDate());
                return (
                  <button key={idx}
                    ...
                    <span className={`text-[10px] font-bold leading-none
                      ${done ? 'text-green-400' : isTodayCell ? 'text-primary' : theme.textMuted}`}>
                      {day}
                    </span>
                  </button>
                );
