...
RESTORE_RECOVERY_INTENT = "restore-confirmed"
...
def _require_destructive_recovery_intent(request: Request, expected: str) -> None:
    if request.headers.get(DESTRUCTIVE_RECOVERY_INTENT_HEADER) != expected:
        raise HTTPException(status_code=428, detail="Explicit recovery intent required")


def require_reset_recovery_intent(request: Request) -> None:
    _require_destructive_recovery_intent(request, RESET_RECOVERY_INTENT)


def require_restore_recovery_intent(request: Request) -> None:
    _require_destructive_recovery_intent(request, RESTORE_RECOVERY_INTENT)
...
def _workout_result(value: dict) -> JSONResponse:
    ...
    if code in {"CAPABILITY_DISABLED", "AUTHORITY_RESET_FENCED"}:
        ...
        "ENTITY_ALREADY_EXISTS", "AUTHORITY_EVIDENCE_MISSING",
...
def validate_restore_ownership(payload: RestorePayload, user_id: str) -> None:
    for table in RESTORE_TABLE_FIELDS:
        for row in getattr(payload, table):
            payload_user_id = row.get("user_id")
...
def validate_restore_existing_id_ownership(
    ...
    for table in RESTORE_TABLE_FIELDS:
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail="Restore ID ownership could not be verified") from error
        if rows is None:
            rows = []
        ...
        for row in rows:
            if not isinstance(row, dict) or not _non_empty_text(row.get("id")) or not _non_empty_text(row.get("user_id")):
                raise HTTPException(status_code=503, detail="Restore ID ownership could not be verified")
            if row["id"] not in ids or row["id"] in existing_ids:
                raise HTTPException(status_code=503, detail="Restore ID ownership could not be verified")
